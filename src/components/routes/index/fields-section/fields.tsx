...
const allWords = async () => {
    ...
    const {data} = await axios.get(http://alp.thescenius.com/api/v1/words, config);
    let words = data.body;
    const wordsWithOutBody = words.filter(
        (word) => !word.body ||  word.body.length === 0,
    );
    // get data for description
    for (let i = 0; i < wordsWithOutBody.length; i++) {
        try {
            const {data} = await axios.get(https://api.dictionaryapi.dev/api/v2/entries/en/${wordsWithOutBody[i].title}, config);
            const word = data[0];
            // const description = word.meanings.reduce(
            //     (acc, meaning) => {
            //         return acc + meaning.definitions[0];
            //     },
            //     "",
            // );
            console.log("meanings => ",word.meanings);
            const description = word.meanings[0].definitions[0];
            const index = wordsWithOutBody.findIndex(
                (word) => word.title === word.title,
            );
        wordsWithOutBody[index].body = description;
        } catch (error) {
            console.log(error);
            wordsWithOutBody[i].body = "";
        }
    }
    words = words.filter(
        (word) => word.body && word.body.length > 0,
    );
    words = [...words,...wordsWithOutBody];
    
    console.log("words in context => ",words);
    dispatch({
      type: SHOW_WORDS,
      words: words
    });
